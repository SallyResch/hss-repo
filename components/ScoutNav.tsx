    ...
    return (
        <>
            ...
            <nav className="hidden md:flex flex-row gap-5 items-center">
                {items.map((item) => (
                    <a key={item.path} href={item.path} target="_blank" rel="noopener noreferrer" className="font-bold text-hss-lightblue hover:border-b-2 border-hss-lightblue">{item.label}</a>
                ))}
            </nav>
